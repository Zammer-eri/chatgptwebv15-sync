...
struct UserAgentPolicy {
    ...
    func configuration(for url: String, prefersDesktopMode: Bool) -> UserAgentConfiguration {
        ...
        let shellForcesAndroidUserAgent = ShellConfig.current.userAgentPolicy == .androidMobile
        let configurableShellUsesAndroidUserAgent = ShellConfig.current.userAgentPolicy == .configurable &&
            Prefs.CompatibilitySettings.useAndroidUserAgent

        if (shellForcesAndroidUserAgent || configurableShellUsesAndroidUserAgent) && !prefersDesktopMode,
        ...
        let usesAndroidUserAgent = shellForcesAndroidUserAgent ||
            configurableShellUsesAndroidUserAgent ||
            (ShellConfig.current.target == .browser && Prefs.CompatibilitySettings.useAndroidUserAgent) ||
            (host.map { host in
