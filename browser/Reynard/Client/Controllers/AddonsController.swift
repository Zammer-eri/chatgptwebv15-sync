...
final class AddonsController: NSObject, AddonEmbedderDelegate {
    ...
    func start() async {
        ...
        do {
            ...
            prepareVisibleAddonIcons()
            currentSession()?.setAddonTabActive(true)
            controller?.refreshAddressBar()
        } catch {
            NSLog("%@", "Add-ons startup failed: \(error)")
        }
    }
    ...
    func handleExternalResponse(_ response: ExternalResponseInfo) -> Bool {
        ...
        Task { @MainActor [weak self] in
            do {
                let addon = try await AddonsRuntimeController.shared.install(
                    url: response.url,
                    installMethod: .manager
                )
                self?.presentAlert(
                    title: "Add-on Installed",
                    message: addon.metaData.name ?? addon.id
                )
            } catch {
    ...
    func handleTabSelectionChange(selectedIndex: Int, previousIndex: Int?) {
        if let previousIndex,
           let previousSession = controller?.tabManager.tabs[safe: previousIndex]?.session {
            previousSession.setAddonTabActive(false)
        }

        if let session = controller?.tabManager.tabs[safe: selectedIndex]?.session {
            session.setAddonTabActive(true)
        }
    }
    ...
    func visibleMenuItemsForCurrentSite() -> [AddonMenuItem] {
        ...
        return AddonsRuntimeController.shared.installedAddons.flatMap { addon in
            let actions = visibleActions(for: addon, session: session)
            if !actions.isEmpty {
            ...
            return actions.map { action in
                AddonMenuItem(
                    addon: addon,
                    action: action,
                    title: action.title ?? addon.metaData.name ?? "Add-on"
                )
            }
        }
    }
    ...
    func prepareVisibleAddonIcons() {
        for addon in AddonsRuntimeController.shared.installedAddons {
            prefetchIconIfNeeded(for: addon)
        }
    }
}
