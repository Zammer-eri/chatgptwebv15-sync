...
final class TabManagerImplementation: NSObject, TabManager {
    ...
    init(
        ...
        self.store = ShellConfig.current.features.restoresPreviousTabs ? store : nil
        self.faviconStore = ShellConfig.current.features.loadsFavicons ? faviconStore : nil
        self.historyStore = ShellConfig.current.features.recordsBrowsingHistory ? historyStore : nil
    }
    ...
    private func loadURL(_ url: String, in tab: Tab) {
        loadURL(url, in: tab, flags: GeckoSessionLoadFlags.none)
    }

    private func loadURL(_ url: String, in tab: Tab, flags: Int) {
    ...
    private func scheduleFaviconUpdate(forTabAt index: Int, mode: TabMode? = nil) {
        ...
        faviconTasks[tabID] = Task { [weak self] in
            ...
            let image = await faviconStore.favicon(for: url)
    ...
    func restoreRecentlyClosedTab(id: UUID) -> Bool {
        guard let store,
              let snapshot = store.takeRecentlyClosedTab(id: id) else {
            return false
        }
    ...
    func createInitialTab(openingScreen: HomepageOpeningScreen) {
        if ShellConfig.current.features.loadDefaultURLOnFirstLaunch,
           let defaultURL = ShellConfig.current.defaultURL?.absoluteString {
            if regularTabs.isEmpty && privateTabs.isEmpty {
                let index = addTab(selecting: true, windowId: nil, at: nil, isPrivate: false)
                browse(to: defaultURL, in: regularTabs[index])
    ...
    func reload(_ tab: Tab) {
        ...
        sessionManager.updateSettings(of: tab.session, for: target, tabID: tab.id)
        tab.session.reload()
    }
    ...
    func recover(_ tab: Tab) {
        ...
        tab.state.restoreState = .none
        tab.state.displayState = .pending(recoveredURL)
        tab.state.suppressInitialNavigation = false
        tab.state.sessionNavigationAvailability = .unavailable
        tab.state.navigationState = NavigationAvailability(canGoBack: false, canGoForward: false)
        tab.state.loadingState = .idle

        if selectedTab === tab {
            sessionManager.activate(newSession)
            delegate?.tabManager(self, didReplaceSelectedSession: oldSession, with: newSession)
        }

        delegate?.tabManagerDidChangeTabs(self)
        notifyUpdate(at: location.index, mode: location.mode, reason: .location)
        notifyUpdate(at: location.index, mode: location.mode, reason: .navigationState)
        sessionManager.discard(oldSession, forTab: tab.id, keepingHistory: false)
        loadURL(recoveredURL, in: tab, flags: GeckoSessionLoadFlags.replaceHistory)
        persistState()
    }
...
extension TabManagerImplementation: ContentDelegate {
    ...
    func onSlowScript(session: GeckoSession, scriptFileName: String) async -> SlowScriptResponse {
        ...
        let tab = tabs(for: location.mode)[location.index]
        switch tab.state.displayState {
        case let .pending(value) where isChatGPTURL(value):
            return .resume
        default:
            break
        }

        return isChatGPTURL(tab.url) ? .resume : .halt
    }
